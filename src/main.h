...
static const int COINBASE_MATURITY_JC = 500;
...
static const uint256 hashGenesisBlockOfficial("0x000000001c28a568af6700dcb3632f055e155867f1ea152cfb5d294c216802e7");
...
public:
    ...
    {
        // jcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // jcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // jcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // jcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // jcoin: whether transaction is a coinstake
    bool fCoinStake;

    // jcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // jcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // jcoin
            undo.nTime = nTime;            // jcoin
        }
        return true;
    }
...
public:
    ...
    // jcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // jcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // jcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // jcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // jcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (jcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // jcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // jcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // jcoin: block index flags
    enum
    {
